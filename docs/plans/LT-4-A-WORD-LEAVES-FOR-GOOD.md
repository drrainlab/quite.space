...
- **S5b — перевёрнутые и летучие кадры едут один раз (телефон владельца, rc14,
  2026-09-23 01:37–01:46):** проходы outbox шли подряд каждые ~15 с
  (`spaces=26 express=52 bulk=15 held=19`), и слово снова ехало в хвосте
  полного прохода. Механика: 19 пространств «на догадке» не двигают `lastLen`
  и перепроходятся каждый раз; у каждого ящика при этом было тело, потому что
  (а) **flipped**-кадр (NoCustody/истёкший, ставший структурой — presence
  тестера, устаревшие показания приборов) ехал «в каждый push независимо от
  курсора», (б) **fleeting**-кадр (живое presence) ехал в каждый ящик каждый
  проход — курсор его не помнил; каждый Put в ящик сиблинга звонил ему, тот
  отвечал, звонок (`kickRelaySync → kickOutbox`) запускал следующий полный
  проход. Теперь: `flipAt[i]` = индекс лога кадра, сделавшего его структурой;
  flipped едет, только если `flipAt ≥ from`; fleeting-кадры несут индекс
  (`fleetingIdx`) и раскладываются по группе курсора (`offerGroup.fleet`),
  «NOTHING TO PUT» учитывает и их. Тест `TestAFlippedFrameRidesOnce`
  (квитанции боба выключены — пол по квитанции прятал бы собственный кадр;
  на телефоне ехали ЧУЖИЕ): 3 прохода = 3 Put на старом коде, 0 на новом.
- **S1c — медиа отдельным каналом (телефон владельца, 1.0.26/rc15, 2026-09-23
  11:31: «отправил скриншоты — доставка обычных сообщений тупит»; владелец:
  «для медиа можно отдельный канал, на твоё усмотрение»).** Лог: карточка фото
  ушла курьеру на 7 ящиков (`bulk=7`) — устройство собеседника угадано на трёх
  официальных реле + свои устройства, два скриншота = ~14 МБ вверх по
  мобильному каналу; проход слова в этой комнате — `held=1` («sending
  history»). И пустые проходы (express=0 bulk=0) по 10–58 с без объяснения.
  Сделано: (1) `collectBlobs` возвращает манифесты и медиа-байты РАЗДЕЛЬНО;
  манифесты едут с кадрами, байты — `offerGroup.media` → отдельные задания
  курьера (`copyJob.media`, заявка `offerKey{…, ep+"|media"}`, курсор не
  двигают); линия кадров выбирается по размеру кадров, а не байтов; на пути
  цикла и ручного verb'а медиа по-прежнему синхронно (ride-ahead перед Close);
  (2) **байты никогда не едут в угаданные ящики** (`!guessed[dev]`) — кадры
  идут, байты ждут заявленного маршрута или запроса по требованию;
  (3) blob'ы читаются с диска лениво — только когда у получателя есть тела
  (раньше все манифесты всех ассетов каждого пространства читались под r.mu
  каждым проходом, и ride-ahead «сгорал» на проходе, который ничего не нёс);
  (4) строка `outbox: pass … (lock=… prep=… plan=… send=…)` — фазы прохода,
  чтобы следующий 58-секундный проход назвал виновника. Тест
